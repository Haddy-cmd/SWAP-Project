...
export default function AdminReportsPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-gold-600">Exports</p>
        <h1 className="mt-1 font-serif text-3xl font-medium text-ink-950">Generate Reports</h1>
      </div>

      <div className="grid items-start gap-5 lg:grid-cols-[372px_1fr]">
        {/* ── CONFIG PANEL ─────────────────────────────────────────── */}
        <div className="no-print space-y-6 rounded-2xl border border-ink-200 bg-white p-6 shadow-[0_2px_8px_rgba(19,36,26,.04)]">
          <div>
            <p className="mb-3 text-xs font-bold uppercase tracking-[0.06em] text-ink-400">1 · Report Type</p>
          ...
          <button onClick={download} disabled={csv.isPending || !preview}
            className="flex h-[50px] w-full items-center justify-center gap-2 rounded-xl text-[15px] font-semibold text-ink-25 shadow-[0_12px_24px_rgba(22,69,43,.26)] transition-opacity hover:opacity-95 disabled:opacity-50"
        ...
        <div>
          <div className="no-print mb-3 flex items-center justify-between">
            <div className="flex items-center gap-2 text-[13.5px]">
              <Eye className="h-[18px] w-[18px] text-gold-600" />
              <span className="font-bold text-ink-700">Live Preview</span>
              <span className="text-ink-400">· updates with your selections {isFetching && '…'}</span>
            </div>
            <span className="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-[11.5px] font-bold"
              style={format === 'PDF' ? { color: '#B42318', background: '#E3EEE5' } : { color: '#196B53', background: '#EFF8F4' }}>
          ...
          <div className="rounded-2xl border border-ink-300 bg-ink-100 p-5 sm:p-6 print:border-0 print:bg-white print:p-0">
            {!preview ? (
              <div className="h-[560px] animate-pulse rounded-lg bg-ink-200/60" />
            ) : format === 'PDF' ? (
              /* ---- PDF DOCUMENT ---- */
              <div className="report-doc mx-auto max-w-[820px] rounded-md bg-white p-8 shadow-[0_12px_40px_rgba(19,36,26,.24)] sm:p-11 print:max-w-none print:rounded-none print:p-0 print:shadow-none">
                ...
                <h2 className="mb-1 mt-5 font-serif text-[26px] font-semibold text-ink-950">{preview.title}</h2>
                <div className="mb-6 flex flex-wrap gap-x-5 gap-y-1 text-[12.5px] text-ink-500">
                  <span><b className="font-semibold text-ink-600">Academic Year:</b> {year}</span>
                  <span><b className="font-semibold text-ink-600">Semester:</b> {semester}</span>
                  <span><b className="font-semibold text-ink-600">Generated:</b> {genDate}</span>
                  <span><b className="font-semibold text-ink-600">Prepared by:</b> {user?.name ?? 'Admin'}</span>
                </div>
                ...
                <div className="mt-6 overflow-x-auto"><table className="w-full border-collapse">
                  <thead>
                    <tr>
                      {headers.map((h) => (
                        <th key={h} className="whitespace-nowrap border-b-2 border-ink-300 px-2 py-2 text-left text-[10.5px] font-bold uppercase tracking-wide text-ink-500">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {rows.slice(0, 40).map((r, ri) => (
                      <tr key={ri}>
                        {r.map((c, ci) => (
                          <td key={ci} className="whitespace-nowrap border-b border-ink-100 px-2 py-2 text-[11.5px] text-ink-700">
              ...
              <div className="mx-auto max-w-[900px]">
                <div className="flex items-center gap-2 rounded-t-[10px] border border-ink-300 bg-white px-4 py-2.5">
                  <Table2 className="h-[18px] w-[18px] text-success-700" />
                  <span className="text-[13px] font-semibold text-ink-700">{type}-{year}-{semester.replace(/\s+/g, '')}.csv</span>
                  <span className="ml-auto text-[11.5px] text-ink-400">{rows.length + 1} rows × {headers.length} cols</span>
                </div>

                <div className="min-h-[344px] overflow-auto border border-t-0 border-ink-300 bg-white">
                  {sheetTab === 'data' ? (
                    <table className="w-full border-collapse">
                      <thead>
                        <tr>
                          <th className="w-[34px] border border-ink-300 bg-ink-100 p-1.5" />
                          {headers.map((_, i) => (
                            <th key={i} className="border border-ink-300 bg-ink-100 px-2.5 py-1.5 text-center text-[10.5px] font-semibold text-ink-500">{colLetter(i)}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        <tr>
                          <td className="border border-ink-300 bg-ink-100 p-1.5 text-center text-[10.5px] font-semibold text-ink-500">1</td>
                          {headers.map((h) => (
                            <td key={h} className="whitespace-nowrap border border-ink-200 bg-ink-50 px-2.5 py-1.5 text-[12px] font-bold text-ink-950">{h}</td>
                          ))}
                        </tr>
                        {rows.map((r, ri) => (
                          <tr key={ri}>
                            <td className="border border-ink-300 bg-ink-100 p-1.5 text-center text-[10.5px] font-semibold text-ink-500">{ri + 2}</td>
                            {r.map((c, ci) => (
                              <td key={ci} className="whitespace-nowrap border border-ink-200 px-2.5 py-1.5 text-[12px] text-ink-700" style={ci === 0 ? { fontWeight: 600 } : undefined}>
                                {cellText(c)}
                              </td>
                            ))}
                          </tr>
                    ...
                    <div className="p-6 sm:p-7">
                      <p className="text-[15px] font-bold text-ink-950">{chart.title}</p>
                      <p className="mb-5 text-[12.5px] text-ink-500">{chart.sub}</p>
                      ...
                      <p className="mt-6 text-[11px] text-ink-350">Chart preview · the CSV download contains the data sheet only.</p>
                    </div>
                  )}
                </div>

                <div className="flex items-center gap-1 rounded-b-[10px] border border-t-0 border-ink-300 bg-ink-100 px-2 pt-1.5">
  ...
  return (
    <div className="relative">
      ...
      <button onClick={() => setOpen((o) => !o)}
        className="flex h-[46px] w-full items-center justify-between rounded-[11px] border border-ink-200 bg-ink-50 px-3.5 text-sm font-semibold text-ink-900">
        {value} <ChevronDown className="h-[18px] w-[18px] text-ink-400" />
      </button>
      {open && (
        <>
          <div className="fixed inset-0 z-20" onClick={() => setOpen(false)} />
          <div className="absolute left-0 right-0 top-[74px] z-30 rounded-xl border border-ink-200 bg-white p-1.5 shadow-[0_18px_40px_rgba(19,36,26,.18)]">
            {options.map((o) => {
              const on = o === value
              return (
                <button key={o} onClick={() => { onChange(o); setOpen(false) }}
                  className="flex w-full items-center justify-between rounded-lg px-3 py-2.5 text-[13.5px] text-ink-900"
                  style={{ background: on ? '#FDF8E4' : 'transparent', fontWeight: on ? 700 : 500 }}>
                  {o} {on && <Check className="h-[17px] w-[17px] text-brand-700" />}
                </button>
              )
            })}
          </div>
...
function FormatCard({ on, Icon, name, desc, onClick }: { on: boolean; Icon: LucideIcon; name: string; desc: string; onClick: () => void }) {
  return (
    <button onClick={onClick} className="flex flex-col items-center gap-1.5 rounded-xl px-3 py-4 text-center transition-colors"
      style={{ border: on ? '1.5px solid #1F5B3A' : '1px solid #DCE0CF', background: on ? '#E3EEE5' : '#F7F6EE' }}>
      <Icon className="h-6 w-6" style={{ color: on ? '#1F5B3A' : '#8C968F' }} />
      <span className="text-[13.5px] font-bold text-ink-950">{name}</span>
...
function StatGrid({ stats, compact }: { stats: { label: string; value: string }[]; compact?: boolean }) {
  ...
  return (
    <div className={`grid grid-cols-2 gap-px overflow-hidden rounded-[10px] border border-ink-200 bg-ink-200 sm:grid-cols-4 ${compact ? '' : ''}`}>
      {stats.map((s) => (
        <div key={s.label} className="bg-ink-50 px-4 py-3">
          <p className="mb-1 text-[10.5px] font-bold uppercase tracking-wide text-ink-400">{s.label}</p>
          <p className="font-serif text-[22px] font-semibold text-ink-950">{s.value}</p>
        </div>
      ))}
    </div>
  )
...
function SheetTab({ on, Icon, label, onClick }: { on: boolean; Icon: LucideIcon; label: string; onClick: () => void }) {
  return (
    <button onClick={onClick} className="flex items-center gap-1.5 rounded-t-lg px-4 py-2 text-[12.5px]"
      style={{ background: on ? '#fff' : 'transparent', border: on ? '1px solid #CAD2BC' : '1px solid transparent', borderBottom: 'none', color: on ? '#1F5B3A' : '#6F7B74', fontWeight: on ? 700 : 500 }}>
      <Icon className="h-[15px] w-[15px]" style={{ color: on ? '#1F5B3A' : '#8C968F' }} /> {label}
    </button>
  )
}
    ...
    <div className="flex flex-col gap-3">
      {bars.map((b, i) => (
        <div key={i} className="flex items-center gap-3.5">
          <span className="w-[150px] flex-none truncate text-[12.5px] font-semibold text-ink-600">{b.label}</span>
          <div className="h-[26px] flex-1 overflow-hidden rounded-[7px] bg-ink-100">
            <div className="h-full rounded-[7px]" style={{ width: `${(b.value / max) * 100}%`, background: b.color }} />
          </div>
          <span className="w-[52px] flex-none text-right text-[12.5px] font-bold text-ink-700">{b.value}{unit ?? ''}</span>
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div className="flex flex-wrap items-center gap-9">
      <div className="flex h-[172px] w-[172px] flex-none items-center justify-center rounded-full"
        style={{ background: `conic-gradient(#1F8163 0 ${relPct}%, #F59E0B ${relPct}% 100%)` }}>
        <div className="flex h-[108px] w-[108px] flex-col items-center justify-center rounded-full bg-white">
          <span className="font-serif text-[30px] font-semibold text-ink-950">{total}</span>
          <span className="text-[10.5px] text-ink-400">recipients</span>
        </div>
      </div>
      <div className="flex flex-1 flex-col gap-3">
        <LegendRow color="#1F8163" label="Released" value={String(released)} />
        <LegendRow color="#F59E0B" label="Pending" value={String(pending)} />
      </div>
    </div>
  )
}
function LegendRow({ color, label, value }: { color: string; label: string; value: string }) {
  return (
    <div className="flex items-center gap-2.5">
      ...
      <span className="flex-1 text-[13px] font-semibold text-ink-700">{label}</span>
      <span className="text-[13px] text-ink-500">{value}</span>
    </div>
  )
}
...
function computeChart(type: TypeKey, rows: (string | number | null)[][], headers: string[]): ChartSpec {
  ...
  if (type === 'applications') {
    ...
    return {
      ...
      bars: [
        { label: 'Approved', value: approved, color: '#1F8163' },
        { label: 'Pending Review', value: pending, color: '#F59E0B' },
        { label: 'Rejected', value: rejected, color: '#E2483B' },
      ],
    }
  }
  if (type === 'recipients') {
    const vi = col('Verified Hours'); const ni = col('Recipient')
    const bars = rows.slice(0, 8).map((r) => ({ label: String(r[ni] ?? ''), value: Number(r[vi]) || 0, color: '#1F5B3A' }))
  ...
  const palette = ['#1F5B3A', '#D4AE22', '#A31A1E', '#2F5D8A', '#1F8163', '#6B4E9A']
  return {
